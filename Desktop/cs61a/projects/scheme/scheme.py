...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        check_procedure(operator) #check if operator is procedure
        if isinstance(operator, MacroProcedure):
            return scheme_eval(operator.apply_macro(rest, env), env)
        arguments=rest.map(lambda operand: scheme_eval(operand, env)) #Evaluate operands
        return scheme_apply(operator, arguments, env) #apply operator to operands
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if scheme_length(formals) != scheme_length(vals):
            raise SchemeError('unequal number of arguments')
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 12
        new_call_frame = self.env.make_child_frame(self.formals, args)
...
def do_if_form(expressions, env):
    ...
    elif scheme_length(expressions) == 3:
...
def do_and_form(expressions, env):
    ...
    if scheme_falsep(value): #if the evaluated expression is false
        return False
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        one_binding=bindings.first
        check_form(one_binding,2,2)
        symbol=one_binding.first
        corresponding_expression=one_binding.second #gets the second pair containing the expression
        value=scheme_eval(corresponding_expression.first,env) #get the information in the second pair and evaluates
        values=Pair(value, values)
        parameters=Pair(symbol, parameters)
        bindings=bindings.second
        check_formals(parameters)
    return env.make_child_frame(parameters, values)
    # END PROBLEM 15
...
def do_define_macro(expressions, env):
    ...
    # BEGIN Problem 21
    check_form(expressions, 2)
    check_formals(expressions.first) #check symbol (name)
    check_form(expressions.first, 0) #check Pair form
    macro_form=expressions.first.first #name of macro
    env.define(macro_form, MacroProcedure(expressions.first.second, expressions.second, env)) #expressions.first.second=formals, expressions.second=body
    return macro_form
    # END Problem 21
...
class MuProcedure(Procedure):
    ...
    def make_call_frame(self, arguments, env):
        return env.make_child_frame(self.formals,arguments)
    # END PROBLEM 16
...
class Promise(object):
    ...
    def evaluate(self):
        if self.expression is not None:
            value = scheme_eval(self.expression, self.env.make_child_frame(nil, nil))
            if not scheme_valid_cdrp(value):
                raise SchemeError("result of forcing a promise should be a pair or nil")
            self.value = value
