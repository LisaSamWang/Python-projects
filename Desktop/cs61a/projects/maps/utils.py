...
def map_and_filter(s, map_fn, filter_fn):
    ...
    """
    # BEGIN Question 0
    return [map_fn(_) for _ in s if filter_fn(_)==True]
    # END Question 0
...
def key_of_min_value(d):
    ...
    """
    # BEGIN Question 0
    return min(d.keys(), key=lambda _:d[_])
    # END Question 0
...
def enumerate(s, start=0):
    ...
    """
    # BEGIN Question 0
    return zip(range(start,start+len(s)),s) #zip does the job by itself, no need 'for' statement
    # END Question 0
...
def mean(s):
    ...
    """
    # BEGIN Question 1
    assert len(s)!=0, 'Sequence cannot be empty'
    return sum(s)/len(s)
    # END Question 1
