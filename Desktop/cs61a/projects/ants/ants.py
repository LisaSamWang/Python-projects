...
class Place(object):
    ...
    def __init__(self, name, exit=None):
        ...
        # BEGIN Problem 2
        if self.exit is not None: #If the Place HAS an EXIT
            exit.entrance=self #let the EXIT'S ENTRANCE be that Place. remember: self=Place. READ: let the next place's entrance be the last place's exit
        # END Problem 2
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                if self.ant.can_contain(insect): #if container ant is in place
                    self.ant.contain_ant(insect)
                elif insect.can_contain(self.ant): #container ant being added
    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            # BEGIN Problem 13
            if isinstance(insect, QueenAnt) and insect.Real_queen: #insect is instance of QueenAnt class
                return
            # END Problem 13
...
class Bee(Insect):
    ...
    name = 'Bee'
    damage = 1
    is_watersafe=True #bees can fly
    was_scared=False
    ...
    def blocked(self):
        ...
        # BEGIN Problem 7
        if self.place.ant == None or (self.place.ant and self.place.ant.blocks_path == False):
            return False
        else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        place_check=self.place
        counting=0
        while place_check!=hive:
            if place_check.bees and self.min_range<=counting<=self.max_range:
                return random_or_none(place_check.bees)
            place_check=place_check.entrance
            counting+=1
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 5
        FireAntPlace = self.place
        Ant.reduce_armor(self,amount)
        if self.armor<=0: #if it attacks ant
            [bee.reduce_armor(self.damage) for bee in list(FireAntPlace.bees)]
        # END Problem 5
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting>0:
            ...
            eaten_bee=random_or_none(self.place.bees)
...
class NinjaAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 7
        [each_bee.reduce_armor(self.damage) for each_bee in list(self.place.bees)]
        # END Problem 7

# BEGIN Problem 8
# The WallAnt class
class WallAnt(Ant):
    """WallAnt does nothing in each turn"""
    name='Wall'
    implemented=True
    food_cost=4
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, colony):
        ...
        [bee.reduce_armor(self.damage) for bee in list(self.place.bees)]
        # END Problem 10
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        ...
        if QueenAnt.Real_queen==True: #we already have a queen
            self.Real_queen=False
        else:
            QueenAnt.Real_queen=True #one true queen initialized
        # END Problem 13
    ...
    def action(self, colony):
        ...
        # BEGIN Problem 13
        if self.Real_queen:
            place_check = self.place.exit
            while place_check: #while place checked is not None
                if place_check.ant:
                    if place_check.ant not in self.is_doubled:
                        place_check.ant.damage*=2
                        self.is_doubled+=[place_check.ant]
                    if place_check.ant.is_container and place_check.ant.contained_ant and place_check.ant.contained_ant not in self.is_doubled:
                        place_check.ant.contained_ant.damage*=2
                        self.is_doubled+=[place_check.ant.contained_ant]
                place_check=place_check.exit
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 13
        self.armor-=amount
        if self.Real_queen and self.armor<=0:
            bees_win()
        else:
            self.place.remove_insect(self)
        # END Problem 13
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    turn_counter=0
    with_effect = effect(bee.action,bee)
    normal=bee.action
    def apply(colony): #helper function to bring in colony
        nonlocal turn_counter
        if (effect==make_scare and bee.was_scared==False or effect==make_slow) and duration>turn_counter:
            with_effect(colony)
        else:
            normal(colony)
        turn_counter+=1
        if effect==make_scare and duration<=turn_counter:
            bee.was_scared=True
            bee.ChangeDirection=False
    bee.action=apply
    # END Problem EC
